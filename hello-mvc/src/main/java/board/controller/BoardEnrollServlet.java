...
public class BoardEnrollServlet extends HttpServlet {
	 ...
	 * 
	 * 파일 업로드 절차
	 * 	- 1. 제출폼 enctype="multipart/form-data" => 실제 파일데이터를 전송하기 위함. 작성하지 않으면 파일명만 전송됨
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			// ServletContext application = getServletContext(); // 생명주기가 가장 길다!
			// String webRoot = application.getRealPath("/"); // webapp 디렉토리까지의 경로
			
			// File.separator : 운영체제별 경로 구분자(window : \, mac/linux : /)
			// String saveDirectory = webRoot + "upload" + File.separator + "board"; 
			// String saveDirectory = webRoot + "/upload/board"; // 요즘은 OS별로 구분하지 않아도 됨
			String saveDirectory = getServletContext().getRealPath("/upload/board"); // 한줄로 처리 가능
			...
			// 3. 사용자입력값 처리
			// dto 객체 생성
			// MultipartRequest객체로 처리하는 것 잊지 말기!
